...
export function Flashcard({ question }: { question: Question }) {
    ...
    >
      ...
      >
        <div className="absolute inset-0 flex flex-col justify-between rounded-[4px] bg-panel/95 p-7 [backface-visibility:hidden]">
          <div>
            <div className="font-display text-xs font-bold uppercase tracking-[0.22em] text-scrub">{question.category}</div>
            <h2 className="mt-5 font-display text-3xl font-bold leading-tight text-white">{question.prompt}</h2>
          </div>
        ...
        <div className="absolute inset-0 flex rotate-y-180 flex-col justify-between rounded-[4px] border border-trauma/30 bg-[#170b10] p-7 [backface-visibility:hidden] [transform:rotateY(180deg)]">
          <div>
            ...
            <h2 className="mt-5 font-display text-3xl font-bold leading-tight text-white">{question.answer}</h2>
            <p className="mt-5 text-base leading-7 text-white/70">{question.explanation}</p>
          </div>
